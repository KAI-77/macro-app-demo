    ...
    try {
        ...
        for (const line of lines) {
            if (line.toLowerCase().includes('calories')) {
                formattedRecipe.push(line.replace(/calories:?\s*(\d+).*/i, 'Calories: $1'));
            } else if (line.toLowerCase().includes('protein')) {
                formattedRecipe.push(line.replace(/protein:?\s*(\d+).*/i, 'Protein: $1 g'));
            } else if (line.toLowerCase().includes('carbohydrates')) {
                formattedRecipe.push(line.replace(/carbohydrates:?\s*(\d+).*/i, 'Carbohydrates: $1 g'));
