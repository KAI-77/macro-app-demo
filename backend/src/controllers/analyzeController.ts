...
import { UploadedFile, MimeTypes, UserRequest } from "../types/interface";
...
export const analyzeImage = async (req: UserRequest, res: Response) => {
  let imageUrl = null;
  ...
  try {
    ...
    const supabase = getSupabaseClient(userId.toString());
    const file = req.file;
    ...
    const mimeType =
      mimeTypes[fileExtension as keyof typeof mimeTypes] || "image/jpeg";
